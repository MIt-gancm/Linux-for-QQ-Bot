...
local_version=$(grep -oP 'v\s*:\s*\"\K[^"]+' /gancm/version.txt)
cloud_version=$(grep -oP 'v\s*:\s*\"\K[^"]+' /gancm/newversion.txt)
uncloud_version=$(grep -oP 'un\s*:\s*\"\K[^"]+' /gancm/version.txt)
...
xzkuang(){
    ...
    shift 4
    local options=("$@")
    local choices=$(whiptail --title "$title" --checklist \
        "$prompt" $height $width $(( ${#options[@]} - 4 )) \
        "${options[@]}" \
        3>&1 1>&2 2>&3)
}
...
csh(){
if [ -d /gancm ];then
echo -e "$(gettime)${GREEN}成功匹配到配置文件存放目录${RES}"
else
echo -e "$(gettime)${RED}没有匹配到配置文件存放目录 尝试初始化配置信息${RES}"
mkdir /gancm
touch /gancm/bot.cfg
touch /gancm/cfg.cfg
echo -e '
...
if [ -d /gancm/log ];then
echo -e "$(gettime)${GREEN}成功读取到日志存放目录${RES}"
else 
echo -e "$(gettime)${RED}日志文件夹不存在  尝试创建${RES}"
mkdir /gancm/log
fi
...
echo -e "$(gettime)${GREEN}成功读取到配置文件${RES}"
else 
echo -e "$(gettime)${RED}配置文件不存在  尝试创建${RES}"
touch /gancm/cfg.cfg
fi

if [ -f /gancm/bot.cfg ];then
echo -e "$(gettime)${GREEN}成功读取到BOT配置文件${RES}"
else 
echo -e "$(gettime)${RED}配置文件不存在  尝试创建${RES}"
touch /gancm/bot.cfg
fi
...
if (whiptail --title "运行警告"  --yes-button "退出脚本" --no-button "继续运行"  --yesno "该脚本的大多数内容需要root权限 当前运行环境并非root建议 切换为root权限执行 我们非常建议您使用Root权限运行 如果没有root权限脚本将无法存放配置文件  在必要情况下我们会请求获取root权限" 10 60) then
    echo -e "$(gettime)${GREEN}感谢你的使用 脚本已退出${RES}"
    exit 0
else
    if (whiptail --title "个人习惯配置" --yes-button "不再理会警告" --no-button "每次都要自己选择"  --yesno "你希望下次检测到根除以非root权限运行时怎么做？" 10 60) then
    echo -e "$(gettime)${YELLOW}当前脚本并没有root权限 只能尝试写入${RES}"
    sed -i "s/ROOT:".*"/ROOT:\"0\"/" /gancm/cfg.cfg
    ROOT=$(grep -oP 'ROOT\s*:\s*\"\K[^"]+' /gancm/cfg.cfg)
    if [ "$ROOT" = "0" ]; then
      echo -e "$(gettime)${GREEN}ROOT检测关闭成功${RES}"
    elif [ "$ROOT" = "1" ]; then
      echo -e "$(gettime)${RED}Root检测关闭失败 原因:无法写入${RES}"
    else
      echo -e "$(gettime)${RED}Root检测关闭失败 无法确定原因 可能是因为配置文件不存在 或 Root检查配置数据异常${RES}"    
    fi
else
    echo 
fi
...
echo -e "$(gettime)${GREEN}版本读取完成${RES}"
else
echo -e "$(gettime) ${RED}版本信息不存在或数据异常 尝试初始化默认数据${RES}"
echo -e '
v:"A-1-1"
un:""
' > /gancm/version.txt
...
if [ "$DYC" = "" ]; then 
echo -e "$(gettime) ${RED}配置信息不存在或数据异常 尝试初始化默认数据${RES}"
echo -e '
MRLJ:"gancm"
DYC:"1"
K:""
...
echo -e "$(gettime)${GREEN}脚本读取完成${RES}"
else
echo -e "$(gettime)${RED}异常退出 原因:可能为用户修改了配置文件校验数值
...
if [ "$BOTDYC" = "" ]; then 
echo -e "$(gettime) ${RED}BOT配置信息不存在或数据异常 尝试初始化默认数据${RES}"
echo -e '
BOTDYC:"1"
Yunzai:"0"
xpm:"0"
...
echo -e "$(gettime)${GREEN}BOT信息读取完成${RES}"
else
echo -e "$(gettime)${RED}异常退出 原因:可能为用户修改了配置文件校验数值
   请尝试使用rm -rfv /gancm/* 删除配置文件${RES}"
exit 1
...
if [ "$Yunzai" = "1" ]; then 
echo -e "$(gettime) ${GREEN}Yunzai-Bot成功安装 ${RES}路径为${YELLOW}${YunzaiLJ}${RES}"
elif [ "$Yunzai" = "0" ]; then
echo -e "$(gettime) ${GREEN}Yunzai-Bot未安装"
else
echo -e "$(gettime)${RED}异常退出 原因:可能为用户修改了BOT配置文件
   请尝试使用rm -rfv /gancm/* 删除配置文件${RES}"
exit 1
fi

if [ "$xpm" = "1" ]; then 
echo -e "$(gettime) ${GREEN}小派蒙成功安装 ${RES}路径为${YELLOW}${xpmLJ}${RES}"
elif [ "$xpm" = "0" ]; then
echo -e "$(gettime) ${GREEN}小派蒙未安装"
else
...
if [ "$Yunzai" = "0" ]; then 
if [ "$YunzaiLJ" = "$K" ]; then 
echo -e "$(gettime)${GREEN}Yunzai-Bot非法修改路径 则校验完成${RES}"
else
echo -e "$(gettime)${RED}异常退出 原因:可能为用户在未安装BOT情况下修改了BOT路径文件值
       ${RES} ${YELLOW}请手动修改/gancm/bot.cfg Yunzai的路径 或删除文件再次生成 如果安装了BOT请一定要将 Yunzai: 中0修改为1 ${RES}"
exit 1
fi
else
echo -e "$(gettime)${RED}已安装 跳过校验${RES}"
fi

if [ "$xpm" = "0" ]; then 
if [ "$xpmLJ" = "$K" ]; then 
echo -e "$(gettime)${GREEN}小派蒙非法修改路径 则校验完成${RES}"
else
echo -e "$(gettime)${RED}异常退出 原因:可能为用户在未安装BOT情况下修改了BOT路径文件值
...
echo -e "$(gettime)${RED}已安装跳过校验${RES}"
fi
...
if [ "$1" == Yunzai ]; then
         ...
         echo -e "$(gettime)${GREEN}已为您尝试启动BOT 如果是时雨bot请手动启动go-qchttp${RES}"
         echo -e "$(gettime)${GREEN}前台启动请不要关闭当前shell${RES}"
         ENDTIME=`date +%s`
         ...
         echo -e "$(gettime)${GREEN}后台启动可关闭当前 shell${RES}"
         echo -e "$(gettime)${GREEN}已为您尝试启动BOT 如果是时雨bot请手动启动go-qchttp${RES}"
         ENDTIME=`date +%s`
         DELTATIME=`expr $ENDTIME - $STARTTIME`
         if [ "$DELTATIME" -le "3" ]; then
...
echo -e "$(gettime)${RED}获取BOT运行PID${RES}"
server=`ps -ef | grep "Yunzai-B" | grep -v grep | awk '{print $2}'`
            #如果不存在就重新启动
            if [ "$server" != "$K" ]; then
            kill -9 ${server}
            echo $server
            echo -e "$(gettime)${RED}已尝试结束BOT运行进程${RES}"
            else
            echo -e "$(gettime)${YELLOW}没有找到Yunzai-Bot前台进程${RES}"
            fi
...
server2=`ps -ef | grep "Yunzai-Bot" | grep -v grep | awk '{print $2}'`
            ...
            echo -e "$(gettime)${YELLOW}没有找到Yunzai-Bot后台进程${RES}"
            fi
hcjx
     ...
     cd ${YunzaiLJS} && pnpm run log
         ...
         STARTTIME=`date +%s`
         echo -e "$(gettime)${GREEN}已为您尝试启动小派蒙BOT 请手动启动go-qchttp${RES}"
         echo -e "$(gettime)${GREEN}前台启动请不要关闭当前shell${RES}"        
...
zxpd wget https://${installfor}/MIt-gancm/Linux-for-QQ-Bot/raw/main/newversion.txt -O /gancm/newversion.txt
...
# 检查忽略版本
if [[ "$cloud_version" != "$uncloud_version" ]]; then
     ...
     if [[ "$cloud_version" != "$local_version" ]]; then
         ...
         if [[ "$cloud_type" == "$local_type" ]]; then
             # 分离版本号的日期和数字部分
             cloud_bnum=$(echo "$cloud_version" | cut -d'-' -f2)
             local_bnum=$(echo "$local_version" | cut -d'-' -f2)
             cloud_num=$(echo "$cloud_version" | cut -d'-' -f3)
             local_num=$(echo "$local_version" | cut -d'-' -f3)
             
             if [[ "$cloud_bnum" -gt "$local_bnum" ]]; then
                      echo "有可用更新: $local_version -> $cloud_version"
                      ggbt=$(grep -oP 'ggbt\s*:\s*\"\K[^"]+' /gancm/newversion.txt)
                      ggnr=$(grep -oP 'ggnr\s*:\s*\"\K[^"]+' /gancm/newversion.txt)
                         if [ "$ggbt" != "" -o "$ggnr" != "" ];then
                              printf "%*s\n" $((($(tput cols) + 2) / 2)) "$ggbt"
                              printf "%*s\n" $((($(tput cols) + 2) / 2)) "$ggnr"
                         else
                      ...
                      if [[ "$choice" == "y" ]]; then
                          ...
                          echo "已取消更新"
                          sed -i "s/un:".*"/un:\"$cloud_version\"/" /gancm/version.txt                                                    
                      fi
             else
...
wjjpd(){
dir="$@"
            res=$(ls -A $dir >/dev/null 2>&1 )
               if ! [ -z "$res" ];then
                      echo -e "$(gettime)${YELLOW}文件夹${RES} ${RED}$dir${RES} ${YELLOW}不为空..........${RES}"
                      echo -e "$(gettime)${YELLOW}这会干扰正常的安装步骤${RES}"
                      echo -e "$(gettime) ${RED}是否删除文件夹${RES} ${YELLOW}（y/o/n）${RES}"
                      ...
                      Y | y )
                         ...
                         zxpd rm -rfv ${dir}/
                         zxpd echo -e "$(gettime)${GREEN}删除完成${RES}"
                         ;;
                      N | n )
                         echo -e "$(gettime)${RED}为了不破坏原文件夹 安装程序终止${RES}"
                         exit 1
                         ;;
                      O | o)
                         echo -e "$(gettime)${RED}无视报错强制执行${RES}"
                         ;;
                         esac
               fi
}
...
installxpmbot(){
    if ! [ -x "$(command -v git)" ]; then
        echo -e "$(gettime)${RED}目前并没有安装 Git 正在安装${RES}"
        installapt git
    fi
    ...
    echo -e "$(gettime)${YELLOW} $python ${RES} ${RED} \n 我们需要python大于3.8到3.10的版本 请仔细核对 五秒后开始继续安装${RES}"
    if ! [ -x "$(command -v python3.10)" ]; then
        echo -e "$(gettime)${RED}目前并没有安装 Python 正在安装${RES}"
        installapt  python3.10 
    fi
    if ! [ -x "$(command -v pip3)" ]; then
        echo -e "$(gettime)${RED}目前并没有安装 pip3 正在安装${RES}"
    ...
    case "$wherpyin" in
        *"https://pypi.tuna.tsinghua.edu.cn/simple"*)
            echo -e "$(gettime)${GREEN}当前pip源为:清华大学开源软件镜像站${RES}"
            ;;
        *"https://pypi.mirrors.ustc.edu.cn/simple"*)
            echo -e "$(gettime)${GREEN}当前pip源为:中国科技大学开源软件镜像站${RES}"
            ;;
        *"http://mirrors.aliyun.com/pypi/simple/"*)
            echo -e "$(gettime)${GREEN}当前pip源为:阿里云开源软件镜像站${RES}"
            ;;
        *"http://mirrors.cloud.tencent.com/pypi/simple"*)
            echo -e "$(gettime)${GREEN}当前pip源为:腾讯开源软件镜像站${RES}"
            ;;
        *"http://pypi.douban.com/simple/"*)
            echo -e "$(gettime)${GREEN}当前pip源为:豆瓣开源软件镜像站${RES}"
            ;;
        *)
            while true; do
                echo -e "$(gettime)${YELLOW}当前并非 ${GREEN}豆瓣 腾讯 阿里云 中国科技大学 清华大学${RES}"
                echo -e "$(gettime)${GREEN}1.修改pip源为:清华大学开源软件镜像站${RES}"
                echo -e "$(gettime)${GREEN}2.修改pip源为:中国科技大学开源软件镜像站${RES}"
                echo -e "$(gettime)${GREEN}3.修改pip源为:阿里云开源软件镜像站${RES}"
                echo -e "$(gettime)${GREEN}4.修改pip源为:腾讯开源软件镜像站${RES}"
                echo -e "$(gettime)${GREEN}5.修改pip源为:豆瓣开源软件镜像站${RES}"
                echo -e "$(gettime)${GREEN}0.不修改pip源${RES}"                   
                read -p "是否更改为国内源(0~5)" yes
                case "$yes" in
                    1)
                        echo -e "$(gettime)${GREEN}修改pip源为:清华大学开源软件镜像站${REDS}"
                        zxpd pip3 config set global.index-url https://pypi.tuna.tsinghua.edu.cn/simple
                        break
                        ;;
                    2)
                        echo -e "$(gettime)${GREEN}修改pip源为:中国科技大学开源软件镜像站${RES}"
                        zxpd pip3 config set global.index-url https://pypi.mirrors.ustc.edu.cn/simple
                        break
                        ;;
                    3)
                        echo -e "$(gettime)${GREEN}修改pip源为:阿里云开源软件镜像站${RES}"
                        zxpd  pip3 config set global.index-url http://mirrors.aliyun.com/pypi/simple/
                        break
                        ;;
                    4)
                        echo -e "$(gettime)${GREEN}修改pip源为:腾讯开源软件镜像站${RES}"
                        zxpd pip3 config set global.index-url http://mirrors.cloud.tencent.com/pypi/simple
                        break
                        ;;
                    5)
                        echo -e "$(gettime)${GREEN}修改pip源为:豆瓣开源软件镜像站${RES}"
                        zxpd pip3 config set global.index-url http://pypi.douban.com/simple/
                        break
                        ;;
                    0)
                        echo -e "$(gettime)${GREEN}取消更改${RES}"
                        break
                        ;;
                    *)
                        echo -e "$(gettime)${RED}请输入正确数值${RES}"
                        ;;
                esac
            done
            ;;
    ...
    # 安装playwright相关依赖
    echo -e "$(gettime)${GREEN}安装已完成 在小派蒙目录下使用nb run 启动 ${RES}"     
 ...
 YZI=$(whiptail --title "选择" --menu "选择功能" 15 70 8 \
     ...
     "3" "Yunzai go-cqhtt版" 3>&1 1>&2 2>&3)
        ...
        if ! [ -x "$(command -v wget)" ];then
             ...
             echo -e "$(gettime)${YELLOW}${NODE}${RES}"
            echo -e "$(gettime)${RED}Node.js（版本至少v16以上）yunzai喵喵 或 原版 Redis go版 go-qchttp 请仔细核对 三秒后开始继续安装${RES}"
            ...
            sleep 3s
                    ...
                    if [ ${YZI} = "1" ]; then
                    echo -e "$(gettime)${GREEN}正在拉取"${whereinstall}"中Yunzai-Bot仓库${RES}"
               ...
               echo -e "$(gettime)${RED}目前并没有安装 npm 正在安装${RES}"
               installapt npm 
              fi
                  ...
                  hash -r
                    ...
                    cd Yunzai-Bot
                     if ! [ -x "$(command -v pnpm)" ];then
                         echo -e "$(gettime)${RED}目前并没有安装 pnpm 正在安装${RES}"
                          zxpd npm install pnpm -g
                     fi
                    zxpd pnpm install -P
                    zxpd pnpm add oicq -w
                    if [ ${YZI} = 3 ]; then
                    echo -e "$(gettime)${YELLOW}正在安装go-qchttp${RES}"
                    osType=$(uname -s)
                    ...
                    echo -e "$(gettime)${YELLOW}正在安装redis${RES}"
                    installapt redis 
                    fi
                    echo -e "$(gettime)${GREEN}安装已完成 在Yunzai目录下使用node app 启动 ${RES}"
                    if [ ${YZI} = 3 ]; then
                    echo -e "$(gettime)${GREEN}请先执行一遍 go-cqhttp 选择反向代理
...
chajian(){
     ...
     Xpm)
          if [ ${xpm} = "1" ]; then
               ...
               echo -e "$(gettime)${RED}您当前没有安装小派蒙${RES}"
               ...
               case $yes in
                    Y | y | yes | Yes | YES)
                         ...
                         ;;
                    *)
                         echo -e "$(gettime)${GREEN}感谢您的使用${RES}"
                         hcjx
                         ;;
               esac
          fi
     ...
     Yunzai)
          if [ ${Yunzai} = "1" ]; then
               ...
               echo -e "$(gettime)${RED}您当前没有安装Yunzai-Bot${RES}"
...
-i | --install | i | install)
     ...
     Xpm)
          installxpmbot
              ...
              ;;
     *)
          echo -e "$(gettime)${RED}未知参数 请用-h查看${RES}"
          ;;
     esac
;;
...
*)
              ...
              if ! [ -x "$(command -v lsb_release)" ];then
               echo -e "$(gettime)${RED}目前并没有安装 lsb-core 正在安装${RES}"
               installapt lsb-core
              fi
...
jzbl
     ...
     if [ "${QDML}" = "${K}" ]; then
               cp "${0}" /usr/bin/gancm
               echo -e "$(gettime)没有设置默认启动命令 已初始化为gancm"
               chmod 777 /usr/bin/gancm
               sed -i "s/QDML:".*"/QDML:\"gancm\"/" /gancm/cfg.cfg
     fi
     if [ "${whereinstall}" = "Gitee" ]; then
          ...
          echo -e "$(gettime)${YELLOW}未知安装源 已重置为github${RES}"
          installfor=github.com
     fi
...
##安装源
  if [ "${JBXX}" = "1" ]; then
              ...
              if ! [ -x "$(command -v neofetch)" ];then
               echo -e "$(gettime)${RED}目前并没有安装 neofetch 正在安装${RES}"
               installapt neofetch
              fi
...
##if1
     OPTION=$(whiptail --title "选择安装-脚本版本:$local_version" --menu "选择你要安装的机器人（Yunzai-Bot请使用乌班图18   小派蒙请使用乌班图20 问就是有bug）" 20 50 6 \
     "1" "小派蒙--Bot NB2" \
     "2" "Yunzai-Bot" \
     "3" "待添加" \
     "0" "其他设置" 3>&1 1>&2 2>&3)
     ...
     if [ "$OPTION" = 1 ]; then 
           while true; do
           XPMI=$(whiptail --title "选择" --menu "选择功能" 25 60 13 \
            "1" "安装小派蒙" \
            "2" "启动小派蒙" \
            "3" "启动小派蒙 -- 后台" \
            "4" "解决13579链接失败" \
            ...
            if [ "${XPMI}" = 1 ]; then
             ...
             echo -e ${GREEN}进程已被放置在虚拟终端${YELLOW}screen -r bot${GREEN}查看日志\n进入虚拟终端后 退出终端请点击 ${YELLOW} clrl+ad${GREEN}\n进入虚拟终端后 结束终端请进程点击 ${YELLOW} clrl+d${GREEN}${RES}
              ...
              if ! [ -x "$(command -v iptables)" ];then
               echo -e "$(gettime)${RED}目前并没有安装 iptables 正在安装${RES}"
               installapt iptables
              fi 
              if ! [ -x "$(command -v npm)" ];then
               echo -e "$(gettime)${RED}目前并没有安装 lsof 正在安装${RES}"
               installapt lsof
              fi
              if ! [ -x "$(command -v netstat)" ];then
               echo -e "$(gettime)${RED}目前并没有安装 net-tools 正在安装${RES}"
   ...
   while true; do
     YunzaiBot=$(whiptail --title "选择安装" --menu "选择你要安装的机器人" 27 60 13 \
     "1" "安装Yunzai-Bot" \
     "2" "启动Yunzai-Bot"  \
     "3" "后台启动Yunzai-Bot" \
     "4" "停止Yunzai-Bot" \
...
while true; do
     ...
     if [ "${OPTIONQT}" = 1 ];then
               read -p "输入默认启动名:" cj
               if [ "${cj}" = "${K}" ];then
               echo -e "$(gettime)${RED}启动命令无效 或 未设置启动命令${RES}"
               hcjx
               else
                    if [ "${QDML}" = "${K}" ]; then
                    echo -e "$(gettime)${YELLOW}上一次的启动命令为空值${RES}"
                    else
                    echo -e "$(gettime)${RED}上次启动命令为${QDML}${RES}"                    
                    echo -e "$(gettime)${RED}删除/usr/bin/${QDML}${RES}"
                    rm -rfv /usr/bin/"${QDML}"
                    fi
               cp ${0} /usr/bin/${cj}
               chmod 777 /usr/bin/${cj}
               sed -i "s/QDML:".*"/QDML:\"$cj\"/" /gancm/cfg.cfg
               echo -e "$(gettime)接下来你可以通过使用${GREEN}${cj}${RES}来启动本脚本 \n 如果需要删除，请手动执行 ${RED}rm -rfv /usr/bin/${cj}${RES}"
               echo -e "$(gettime)${GREEN}目前启动命名为${cj}${RES}"
               hcjx
               fi
          ...
          if [ "${exitstatus}" = 0 ]; then
              ...
              echo -e "$(gettime)${GREEN}检查脚本关键词: 『rm』...${RES}"
              grep -r 'rm' ${yisi}*.sh --color=auto "or"
              echo -e "$(gettime)${YELLOW}以上为一场小本信息及关键词标注${RES}"
              echo -e "$(gettime)${RED}这条删除命令 虽然没有什么大问题 但请一定要注意后续删掉的目录 或文件${RES}"
              echo -e 
              echo -e "####################################################################################################"
              
              echo -e "$(gettime)${GREEN}检查脚本关键词: 『su』...${RES}"
              grep -r 'su' ${yisi}*.sh --color=auto "or"
              echo -e "$(gettime)${YELLOW}以上为一场小本信息及关键词标注${RES}"
              echo -e "$(gettime)${RED}他会将用户等级切换为SU 请一定要设置好密码${RES}"
              echo -e "####################################################################################################"
              
              echo -e "$(gettime)${GREEN}检查脚本关键词: 『usr』...${RES}"
              grep -r 'usr' ${yisi}*.sh --color=auto "or"
              echo -e "$(gettime)${YELLOW}以上为一场小本信息及关键词标注${RES}"
              echo -e "$(gettime)${RED}涉及系统目录操作${RES}"
              echo -e "####################################################################################################"
              
              echo -e "$(gettime)${GREEN}检查脚本关键词: 『etc』...${RES}"
              grep -r 'etc' ${yisi}*.sh --color=auto "or"
              echo -e "$(gettime)${YELLOW}以上为一场小本信息及关键词标注${RES}"
              echo -e "$(gettime)${RED}涉及系统目录操作${RES}"
              echo -e "####################################################################################################"
              
              echo -e "$(gettime)${GREEN}检查脚本关键词: 『passwd』...${RES}"
              grep -r 'passwd' ${yisi}*.sh --color=auto "or"
              echo -e "$(gettime)${YELLOW}以上为一场小本信息及关键词标注${RES}"
              echo -e "$(gettime)${RED}涉及更改用户密码${RES}"
              echo -e "####################################################################################################"
              
          
              echo -e "$(gettime)${GREEN}检查脚本关键词: 『kill -9』...${RES}"
              grep -r 'kill -9' ${yisi}*.sh --color=auto "or"
              echo -e "$(gettime)${YELLOW}以上为一场小本信息及关键词标注${RES}"
              echo -e "$(gettime)${RED}强制结束一些进程 一般是用于 解决无响应进程 也可以被不法分子利用来隐藏运行软件${RES}"
              echo -e "####################################################################################################"
              
              echo -e "$(gettime)${GREEN}检查脚本关键词: 『ssh』...${RES}"
              grep -r 'ssh' ${yisi}*.sh --color=auto "or"
              echo -e "$(gettime)${YELLOW}以上为一场小本信息及关键词标注${RES}"
              echo -e "$(gettime)${RED}涉及远程ssh 内网用户无需担心 公网用户，请你一再检查${RES}"
              echo -e "####################################################################################################"
              echo -e "$(gettime)${RED}检查结果均为关键词匹配 自行检查安全性 ${RES}"
              hcjx
          ...
          if [ $exitstatus = 0 ]; then
              ...
              echo -e "$(gettime)${YELLOW}取消修正${RES}."
          fi              
               elif [ "${BOTPZWJ}" = "3" ]; then              
              if (whiptail --title "Yes/No Yunzai安装状态" --yes-button "安装了" --no-button "未安装"  --yesno "脚本开始的基本信息" 10 60) then
          ...
          if [ $exitstatus = 0 ]; then
              ...
              while true; do
               ...
               "0" "返回上级" 3>&1 1>&2 2>&3)
                   ...
                   if [ "${PPA}" = 0 ];then
                   echo -e "$(gettime)${YELLOW}正在安装 PPA必要模块${RES}"
